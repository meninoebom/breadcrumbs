...
def test_list_breadcrumbs_empty(client):
    r = client.post("/api/themes", json={"body_md": "Empty Theme"})
    theme_id = r.json()["id"]

    response = client.get(f"/api/themes/{theme_id}/breadcrumbs")
...
def test_list_breadcrumbs_invalid_theme(client):
    response = client.get("/api/themes/999/breadcrumbs")
    assert response.status_code == 404
...
def test_update_breadcrumb_wrong_theme(client):
    ...
    r1 = client.post("/api/themes", json={"body_md": "Theme A"})
    r2 = client.post("/api/themes", json={"body_md": "Theme B"})
    theme_a_id = r1.json()["id"]
    theme_b_id = r2.json()["id"]

    r = client.post(
        f"/api/themes/{theme_a_id}/breadcrumbs",
        json={"body_md": "Belongs to A"},
    )
    bc_id = r.json()["id"]

    response = client.put(
        f"/api/themes/{theme_b_id}/breadcrumbs/{bc_id}",
...
def test_delete_breadcrumb_wrong_theme(client):
    ...
    response = client.delete(f"/api/themes/{theme_b_id}/breadcrumbs/{bc_id}")
    assert response.status_code == 404

    # Breadcrumb should still exist under the correct theme
    response = client.get(f"/api/themes/{theme_a_id}/breadcrumbs")
...
def test_create_breadcrumb_cross_theme_parent(client):
    ...
    r = client.post(
        f"/api/themes/{theme_b_id}/breadcrumbs",
...
def test_create_breadcrumb_at_max_depth(client):
    ...
    r = client.post("/api/themes", json={"body_md": "Deep Theme"})
    theme_id = r.json()["id"]
    ...
    for i in range(10):
        ...
        r = client.post(f"/api/themes/{theme_id}/breadcrumbs", json=payload)
...
def test_delete_parent_cascades_children(client):
    ...
    # Delete parent
    r = client.delete(f"/api/themes/{theme_id}/breadcrumbs/{parent_id}")
    assert r.status_code == 204

    # Both should be gone
    r = client.get(f"/api/themes/{theme_id}/breadcrumbs")
    assert len(r.json()) == 0
...
def test_update_breadcrumb_ignores_parent_id(client):
    ...
    r = client.put(
        f"/api/themes/{theme_id}/breadcrumbs/{child_id}",
...
def test_delete_theme_cascades_nested_breadcrumbs(client):
    ...
    r = client.post("/api/themes", json={"body_md": "Doomed Theme"})
    theme_id = r.json()["id"]
    ...
    # Delete the theme
    r = client.delete(f"/api/themes/{theme_id}")
    assert r.status_code == 204

    # Theme and all breadcrumbs should be gone
    r = client.get(f"/api/themes/{theme_id}")
